...
namespace eosio_system
{
   ...
   class ux_system_tester : public TESTER
   {
   public:
      void basic_setup()
      {
         ...
         create_accounts({N(eosio.token), N(eosio.ram), N(eosio.ramfee), N(eosio.stake),
                          N(eosio.bpay), N(eosio.saving), N(eosio.names), N(eosio.rex), N(eosio.upay)});
      ...
      void deploy_contract(bool call_init = true)
      {
         ...
         if (call_init)
         {
            base_tester::push_action(config::system_account_name, N(init),
                                     config::system_account_name, mutable_variant_object()("version", 0)("core", UX_CORE_SYM_STR));
         }
      ...
      ux_system_tester(setup_level l = setup_level::full) : TESTER(
                                                                tempdir, [](controller::config &cfg) {
                                                                   cfg.state_size = 1024 * 1024 * 64;
                                                                   cfg.trusted_producers = flat_set<account_name>();
                                                                },
                                                                true)
      {
      ...
      transaction_trace_ptr create_account_with_resources(account_name a, account_name creator, uint32_t ram_bytes = 8000)
      {
         ...
         trx.actions.emplace_back(get_action(config::system_account_name, N(delegatebw), vector<permission_level>{{creator, config::active_name}},
                                             mvo()("from", creator)("receiver", a)("stake_net_quantity", ux_core_sym::from_string("10.0000"))("stake_cpu_quantity", ux_core_sym::from_string("10.0000"))("transfer", 0)));
      ...
      action_result rentcpu(const account_name &from, const account_name &receiver, const asset &payment, const asset &fund = ux_core_sym::from_string("0.0000"))
      {
         return push_action(name(from), N(rentcpu), mvo()("from", from)("receiver", receiver)("loan_payment", payment)("loan_fund", fund));
      }

      action_result rentnet(const account_name &from, const account_name &receiver, const asset &payment, const asset &fund = ux_core_sym::from_string("0.0000"))
      {
      ...
      asset _get_rentrex_result(const account_name &from, const account_name &receiver, const asset &payment, bool cpu)
      {
         ...
         auto trace = base_tester::push_action(config::system_account_name, act, from, mvo()("from", from)("receiver", receiver)("loan_payment", payment)("loan_fund", ux_core_sym::from_string("0.0000")));

         asset rented_tokens = ux_core_sym::from_string("0.0000");
         for (size_t i = 0; i < trace->action_traces.size(); ++i)
         {
      ...
      asset get_rex_fund(const account_name &act) const
      {
         ...
         return data.empty() ? asset(0, symbol{UX_CORE_SYM}) : abi_ser.binary_to_variant("rex_fund", data, abi_serializer_max_time)["balance"].as<asset>();
      }
      ...
      {
         vector<char> data = get_row_by_account(config::system_account_name, config::system_account_name, N(rexbal), act);
         return data.empty() ? ux_core_sym::from_string("0.0000") : abi_ser.binary_to_variant("rex_balance", data, abi_serializer_max_time)["vote_stake"].as<asset>();
      }
      ...
      void setup_rex_accounts(const std::vector<account_name> &accounts,
                              ...
                              const asset &net = ux_core_sym::from_string("80.0000"),
                              const asset &cpu = ux_core_sym::from_string("80.0000"),
                              bool deposit_into_rex_fund = true)
      {
         const asset nstake = ux_core_sym::from_string("10.0000");
         const asset cstake = ux_core_sym::from_string("10.0000");
         create_account_with_resources(N(proxyaccount), config::system_account_name, ram_core_sym::from_string("1.0000"), false, net, cpu);
         BOOST_REQUIRE_EQUAL(success(), push_action(N(proxyaccount), N(regproxy), mvo()("proxy", "proxyaccount")("isproxy", true)));
         for (const auto &a : accounts)
         {
            create_account_with_resources(a, config::system_account_name, ram_core_sym::from_string("1.0000"), false, net, cpu);
      ...
      asset get_balance(const account_name &act, symbol balance_symbol = symbol{UX_CORE_SYM})
      {
      ...
      asset get_balance(std::string_view act, symbol balance_symbol = symbol{UX_CORE_SYM})
      {
      ...
      {
         return stake2votes(ux_core_sym::from_string(s));
      }
      ...
      {
         return get_stats("4," UX_CORE_SYM_NAME)["supply"].as<asset>();
      }
      ...
      abi_serializer initialize_multisig()
      {
         abi_serializer msig_abi_ser;
         {
            ...
            BOOST_REQUIRE_EQUAL(success(), buyram(N(eosio), N(eosio.msig), ux_core_sym::from_string("5000.0000")));
            produce_block();
      ...
      vector<name> active_and_vote_producers()
      {
         ...
         transfer(N(eosio), N(alice1111111), ux_core_sym::from_string("650000000.0000"), config::system_account_name);
         BOOST_REQUIRE_EQUAL(success(), stake(N(alice1111111), N(alice1111111), ux_core_sym::from_string("300000000.0000"), ux_core_sym::from_string("300000000.0000")));
         ...
         //vote for producers
         {
            transfer(config::system_account_name, N(alice1111111), ux_core_sym::from_string("100000000.0000"), config::system_account_name);
            BOOST_REQUIRE_EQUAL(success(), stake(N(alice1111111), ux_core_sym::from_string("30000000.0000"), ux_core_sym::from_string("30000000.0000")));
            BOOST_REQUIRE_EQUAL(success(), buyram(N(alice1111111), N(alice1111111), ram_core_sym::from_string("4.0000")));
      ...
      void cross_15_percent_threshold()
      {
         ...
         regproducer(N(producer1111));
         {
            ...
            trx.actions.emplace_back(get_action(config::system_account_name, N(delegatebw),
                                                ...
                                                mvo()("from", name{config::system_account_name})("receiver", "producer1111")("stake_net_quantity", ux_core_sym::from_string("150000000.0000"))("stake_cpu_quantity", ux_core_sym::from_string("0.0000"))("transfer", 1)));
            ...
            trx.actions.emplace_back(get_action(config::system_account_name, N(undelegatebw),
                                                vector<permission_level>{{N(producer1111), config::active_name}},
                                                mvo()("from", "producer1111")("receiver", "producer1111")("unstake_net_quantity", ux_core_sym::from_string("150000000.0000"))("unstake_cpu_quantity", ux_core_sym::from_string("0.0000"))));
      ...
      fc::variant get_resource_history(uint64_t day) const
      {
         using namespace eosio;
         vector<char> data;
         const auto &db = control->db();

         const auto *t_id = db.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple(N(eosio), N(eosio), N(reshistory)));

         const auto &idx = db.get_index<chain::key_value_index, chain::by_scope_primary>();
         auto itr = idx.lower_bound(boost::make_tuple(t_id->id, day));
      ...
      {
         return push_action(N(eosio), N(initresource), mvo()("dataset_batch_size", dataset_batch_size)("oracle_consensus_threshold", oracle_consensus_threshold)("period_start", period_start)("period_seconds", period_seconds)("initial_value_transfer_rate", initial_value_transfer_rate)("max_pay_constant", max_pay_constant));
      }

      action_result settotalusg(name source, uint64_t total_cpu_us, uint64_t total_net_words, string all_data_hash, time_point_sec period_start)
      {
         return push_action(source, N(settotalusg), mvo()("source", source)("total_cpu_us", total_cpu_us)("total_net_words", total_net_words)("all_data_hash", all_data_hash)("period_start", period_start));
      }
      ...
      action_result addactusg(name source, uint16_t dataset_id, vector<fc::variant> &data, time_point_sec period_start)
      {
         //fc::variant dataset = json_from_file_or_string(data);
         return push_action(source, N(addactusg), mvo()("source", source)("dataset_id", dataset_id)("dataset", data)("period_start", period_start));
      }

      action_result nextperiod(name source)
      {
         //fc::variant dataset = json_from_file_or_string(data);
      ...
      long getSecondsSinceEpochUTC(const string &timestamp)
      {
         ...
         struct tm tm = {0};
         tm.tm_year = year - 1900; // Years from 1900
         tm.tm_mon = month - 1;    // Months from January
         tm.tm_mday = day;
         tm.tm_hour = hour;
         tm.tm_min = min;
         tm.tm_sec = sec;

         time_t timeSinceEpoch = timegm(&tm);
         return timeSinceEpoch;
      }

      void skipAhead(long next_day_seconds)
      {
         time_t current_time = last_block_time();
         time_t diff = difftime(next_day_seconds, current_time);
         time_t next = current_time + diff;

         long blocks = (long)diff * 2;
         produce_block(fc::seconds(diff));
      }

      void dayCycle(string fixture, uint32_t period_start_sec, uint8_t dataset_batch_size)
      {
         time_point_sec period_start = time_point_sec(period_start_sec);
         fc::variant usage_data_vo = json_from_file_or_string(fixture);
      ...
      struct oracle_data
      {
         vector<vector<fc::variant>> usage_datasets;
         string all_data_hash;
         uint64_t total_cpu_usage_us = 0;
         uint64_t total_net_usage_words = 0;
      };

      struct oracle_data generate_all_data_hash(fc::variant all_data, int dataset_batch_size, uint8_t divisor = 1)
      {
         using namespace std;
         ...
         vector<string> usage_dataset_hashes;
         vector<vector<fc::variant>> usage_datasets;

         fc::variants &data_variants = all_data.get_array();
         for (int i = 0; i < data_variants.size(); i = i + dataset_batch_size)
         {
            string individual_usage_string;
            vector<fc::variant> individual_usage_data;
            int max = data_variants.size() > i + dataset_batch_size ? i + dataset_batch_size : data_variants.size();
            for (int j = i; j < max; j++)
            {
               fc::variant_object &vo = data_variants[j].get_object();
               fc::mutable_variant_object mvo = vo;
               individual_usage_string += mvo["a"].as_string() + mvo["u"].as_string();
               total_cpu_usage_us += mvo["u"].as_uint64();
               total_net_usage_words += mvo["net"].as_uint64();
               individual_usage_data.emplace_back(data_variants[j]);
            }
            string individual_usage_hash = fc::sha256::hash((const char *)individual_usage_string.c_str(), individual_usage_string.size());
            usage_dataset_hashes.emplace_back(individual_usage_hash);
            usage_datasets.emplace_back(individual_usage_data);
         }

         string total_usage_string = std::to_string(total_cpu_usage_us) + "-" + std::to_string(total_net_usage_words);
         string total_usage_hash = fc::sha256::hash((const char *)total_usage_string.c_str(), total_usage_string.size());
         usage_dataset_hashes.emplace(usage_dataset_hashes.begin(), total_usage_hash);
         string all_data_hash_string;
         for (int i = 0; i < usage_dataset_hashes.size(); i++)
         {
            all_data_hash_string += usage_dataset_hashes[i];
            if (i != usage_dataset_hashes.size() - 1)
               all_data_hash_string += "-";
         }
         string all_data_hash = fc::sha256::hash((const char *)all_data_hash_string.c_str(), all_data_hash_string.size());
         struct oracle_data usage_data = {usage_datasets, all_data_hash, total_cpu_usage_us, total_net_usage_words};
         return usage_data;
      }
      ...
      fc::variant system_usage_table_info(name oracle)
      {
         name table_name = N(ressysusage);
         vector<char> data = get_row_by_account(config::system_account_name, config::system_account_name, table_name, account_name(oracle));
   ...
   {
      return ux_core_sym::from_string(eos_str).get_amount();
   }
